...
export const FriendBarItemView: FC<{ friend: MessengerFriend }> = props =>
{
    ...
    if(!friend)
    {
        return (
            <div style={{backgroundColor: "var(--test-galaxytwo)"}} ref={ elementRef } className="btn btn-lg friend-bar-item friend-bar-search">
                <img className="position-absolute" style={{left: "2px", top: "2px"}} src="https://swfs.hgalaxy.net/images/head.png"/>
    ...
    return (
        <div style={{backgroundColor: "var(--test-galaxytwo)"}} ref={ elementRef } className={ 'btn friend-bar-item btn-lg ' + (isVisible ? 'friend-bar-item-active' : '') } onMouseOver={e => setVisible(prevValue => !prevValue)} onMouseOut={e => setVisible(prevValue => !prevValue)}>
