        ...
        <div className="putamierda">
            <Grid>
                ...
                <Column size={ 5 } overflow="auto">
                    ...
                    { selectedItem &&
                        <Column grow justifyContent="between" gap={ 1 }>
                            <div>
                                <Text style={{color: "var(--test-galaxytext)"}} variant="none" grow truncate>{ selectedItem.name }</Text>
                            
                                { !!roomSession &&
                                    <Button style={{marginTop: "10px"}} variant="success" onClick={ event => attemptItemPlacement(selectedItem) }>
                                        { LocalizeText('inventory.furni.placetoroom') }
                                    </Button> }
                                { (selectedItem && selectedItem.isSellable) &&
                                    <Button style={{marginTop: "10px"}}  onClick={ event => attemptPlaceMarketplaceOffer(selectedItem) }>
