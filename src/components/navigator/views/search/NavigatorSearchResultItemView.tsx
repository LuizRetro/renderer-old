...
export const NavigatorSearchResultItemView: FC<NavigatorSearchResultItemViewProps> = props =>
{
    ...
    if(thumbnail) return (
        <Column pointer overflow="hidden" alignItems="center" onClick={ visitRoom } gap={ 0 } className="navigator-item rounded-3 small mb-1 flex-column" { ...rest }>
            <Flex className="w-100" style={{backgroundColor: "var(--test-galaxytwo)", padding: "3px", position: "relative", top: "10px",  zIndex: "100", marginLeft: "-1px", marginRight: "-3px"}}>
                <Text variant="white" truncate className="flex-grow-1">{ roomData.roomName }</Text>
    ...
    return (
        <Flex pointer overflow="hidden" alignItems="center" onClick={ visitRoom } gap={ 2 } className="navigator-item px-2 py-2 small" { ...rest }>
